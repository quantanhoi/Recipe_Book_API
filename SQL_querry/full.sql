...
alter table Rezept add constraint FK_REZEPT_REFERENCE_BILD foreign key (B_ID) references Bild (B_ID) on delete cascade on update cascade;

alter table RezeptStep add constraint FK_REZEPTST_REZEPT_RE_REZEPT foreign key (R_ID) references Rezept (R_ID) on delete cascade on update cascade;
